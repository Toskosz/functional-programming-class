...
optimizeE exp = case exp of
  ...
  EComp exp1 exp2 ->
    let optExp1 = optimizeE exp1
        optExp2 = optimizeE exp2
        optEComp = EComp optExp1 optExp2
     in if ((isLiteral optExp1) && (isLiteral optExp2))
          then wrapValueExpression (eval [] optEComp)
          else optEComp
  EIf exp expT expE ->
    let optExp = optimizeE exp
